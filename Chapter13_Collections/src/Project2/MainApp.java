...
//oefening4
        // weg te geven-Maak een ProrityQueue die ‘sortedFriends’eerst + lambda
        //prioriteit geeft aan je familieleden, dan aan je beste vriend en dan pas aan de mensen die je het langste kent.


        PriorityQueue<Friend> sortedFriends = new PriorityQueue<>(
                (friend, friend2) -> {
                    // priority
                    if (friend.isFamily() && !friend2.isFamily()) {
                        return -1;
                    } else if (!friend.isFamily() && friend2.isFamily()) {
                        return 1;
                    } else {
                        return Integer.compare(friend2.getFriendShipLevel(), friend.getFriendShipLevel());
                    }
                }
        );
        ...
        //sorted friends
        System.out.println("Sorted Friends for Giveaway:");
        while (!sortedFriends.isEmpty()) {
            System.out.println(sortedFriends.poll().getName());
        }
